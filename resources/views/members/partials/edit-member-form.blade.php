  ...
  <form method="post" action="{{ route('members.update', $member->id) }}" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
      <div>
        ...
        <x-text-input id="first_name" name="first_name" type="text" :value="old('first_name', $member->first_name)" autocomplete="first_name"
          maxlength="255" />
        <x-input-error :messages="$errors->get('first_name')" />
      </div>
      <div>
        <x-input-label for="last_name" :value="__('Last Name')" />
        <x-text-input id="last_name" name="last_name" type="text" :value="old('last_name', $member->last_name)" autocomplete="last_name"
          maxlength="255" />
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
      <div>
        ...
        <x-text-input id="address" name="address" type="text" :value="old('address', $member->address)" autocomplete="address"
          maxlength="255" />
        <x-input-error :messages="$errors->get('address')" />
      </div>
      <div>
        <x-input-label for="phone" :value="__('Phone')" />
        <x-text-input id="phone" name="phone" type="text" :value="old('phone', $member->phone)" autocomplete="phone"
          maxlength="255" />
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
      <div>
        ...
        <x-text-input id="email" name="email" type="email" :value="old('email', $member->email)" autocomplete="email"
          maxlength="255" />
        <x-input-error :messages="$errors->get('email')" />
      </div>
      <div>
        <x-input-label for="last_visited_date" :value="__('Last Visited Date')" />
        <x-text-input id="last_visited_date" name="last_visited_date" type="date" :value="old('last_visited_date', $member->last_visited_date)"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
      <div>
        ...
        <x-select-input id="clothing_size" name="clothing_size" type="text" :options="$clothingSizes"
          chooseOptionText="Select a Clothing Size" :selectedOption="old('clothing_size', $member->clothing_size)" autocomplete="clothing_size" />
        <x-input-error :messages="$errors->get('clothing_size')" />
      </div>
      <div x-data="imageViewer('{{ $member->picture ? asset('storage/' . $member->picture) : asset('images/no-user.png') }}')">
